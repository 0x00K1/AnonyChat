    	...
    	try {
            ...
            if ("NOPASSWORD".equals(serverResponse)) {
                ...
                if ("WRONGSERVERPASSWORD".equals(serverResponse)) {
                	Main.clearScreen();
                    System.out.println("[x]Wrong server password. Try again.");
                    return false;
                }
            } else {
            ...
            if ("INVALIDUSERNAME".equals(serverResponse)) {
            	Main.clearScreen();
                System.out.println("[x]Invalid Username. Try again.");
                return false;
            } else if ("INVALIDUSER".equals(serverResponse)) {
            	Main.clearScreen();
            	System.out.println("[x]User already exists. Try again with a different username.");
            	return false;
            } else if ("INVALIDSTRINGUSERNAME".equals(serverResponse)) {
            	Main.clearScreen();
            	System.out.println("[x]Username must start with a letter. Try again.");
            	return false;
            } else if ("INVALIDALPHANUMERICUSERNAME".equals(serverResponse)) {
            	Main.clearScreen();
            	System.out.println("[x]Username must contain only letters and numbers. Try again");
            	return false;
