    ...
    private static class Handler extends Thread {
        ...
        public void run() {
            try {
                ...
                while (true){
            		...
            		if (bannedIPs.containsKey(userIP) && !adminUsers.contains(currentUserSession.getRealUsername())) {
            			out.println("BANNED");
            			break;
            		}
                	...
                	if (input == null) {
                	    ...
                	    continue; // Skip processing if the message is empty
                	}
        ...
        private boolean authenticator(Socket socket, BufferedReader in, PrintWriter out) {
        	...
        	try {
        		...
        		if (bannedIPs.containsKey(userIP) && !(Rooter)) {
        			out.println("BANNED");
        			return false;
        		}
        ...
        private void broadcast(String message) {
            if (message != null) {
                ...
                out.println("MESSAGE [SERVER]> Provide a message.");
            }
        }
        ...
        private void updcapa(String updcapacity) {
            try {
                ...
                out.println("MESSAGE [SERVER]> Invalid number format for capacity. Enter a valid number.");
            }
        }
        ...
        private void kick(String username) {
            if (username != null && onlineUsers.containsKey(username) && !adminUsers.contains(username)) {
                ...
                out.println("MESSAGE [SERVER]> Provide a valid username.");
            }
        }
        ...
        private void ban(String username) {
            if (username != null && onlineUsers.containsKey(username) && !adminUsers.contains(username)) {
                ...
                // Disconnect and ban other users with the same IP
                for (Map.Entry<String, String> entry : userIPs.entrySet()) {
                    if (entry.getValue().equals(userIP) && !adminUsers.contains(entry.getKey())) {
                        PrintWriter writer = onlineUsers.get(entry.getKey());
                        ...
                        // Close the user's socket connection
                        try {
                            userSockets.get(entry.getKey()).close();
                        } catch (IOException e) {
                ...
                out.println("MESSAGE [SERVER]> " + username + " and any associated usernames have been banned.");
            } else {
                out.println("MESSAGE [SERVER]> Provide a valid username.");
            }
        }
            ...
            if (username != null && !adminUsers.contains(username)) {
                ...
                if (userIPsToUnban != null) {
                    ...
                    if (isUnbanned) {
                        ...
                        out.println("MESSAGE [SERVER]> " + username + " is not currently banned.");
                    }
                } else {
        ...
        private void shutdown() {
            try {
                for (PrintWriter writer : writers) {
                    ...
                    writer.println("MESSAGE [SERVER]> [#]Server Closed.");
                }
    ...
    private static void serverConfig() {
        ...
        do {
            ...
            if (!Character.isLetter(newRootUsername.charAt(0))) {
                Main.clearScreen();
                System.out.println("[x]Username must start with a letter.");
            }
            // Contains only letters and numbers.
            else if (!newRootUsername.matches("[A-Za-z0-9]+")) {
                Main.clearScreen();
                System.out.println("[x]Username must contain only letters and numbers.");
            }
            // Is reserved ?
            else if (isUsernameReserved(newRootUsername)) {
                Main.clearScreen();
                System.out.println("[x]The chosen username is reserved. Select a different username.");
            }
        } while (!Character.isLetter(newRootUsername.charAt(0)) || !newRootUsername.matches("[A-Za-z0-9]+") || isUsernameReserved(newRootUsername));
